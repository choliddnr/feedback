...
const { data: _analysisResult } = await useFetch<
  { products: Product; analysis: { analysis: string } | null }[]
...
<template>
  ...
  <!-- 
  :key="index" -->
  <div v-for="(_analysis, index) in analysis">
    <!-- <AdminDashboardAnalysis :analysis="_analysis" :key="index" /> -->
    <UPageCard class="p-6 rounded-2xl shadow">
      <pre>{{ _analysis }}</pre>
    </UPageCard>
  </div>
  ...
  <!-- v-for="(_analysis, index) in analysis"
    <AdminDashboardPerProduct :key="index" :product="_analysis" /> -->
  <!-- </ClientOnly> -->
</template>
